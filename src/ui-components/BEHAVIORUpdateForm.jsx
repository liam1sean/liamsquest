...
export default function BEHAVIORUpdateForm(props) {
  const {
    id: idProp,
    bEHAVIOR,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = bEHAVIORRecord
      ? { ...initialValues, ...bEHAVIORRecord }
      : initialValues;
  ...
  const [bEHAVIORRecord, setBEHAVIORRecord] = React.useState(bEHAVIOR);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(BEHAVIOR, idProp)
        : bEHAVIOR;
      setBEHAVIORRecord(record);
    };
    queryData();
  }, [idProp, bEHAVIOR]);
  React.useEffect(resetStateValues, [bEHAVIORRecord]);
  const validations = {
    client_id: [],
    observation_id: [],
    lst_updt_dt: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            BEHAVIOR.copyOf(bEHAVIORRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "BEHAVIORUpdateForm")}
      {...rest}
    >
      <TextField
        label="Client id"
