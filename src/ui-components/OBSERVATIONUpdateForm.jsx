...
export default function OBSERVATIONUpdateForm(props) {
  const {
    id: idProp,
    oBSERVATION,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = oBSERVATIONRecord
      ? { ...initialValues, ...oBSERVATIONRecord }
      : initialValues;
    setObservation_id(cleanValues.observation_id);
    setObservation_dt(cleanValues.observation_dt);
    setFrequency(cleanValues.frequency);
    setDuration(cleanValues.duration);
    setIntensity(cleanValues.intensity);
    setReplacement_behavior(cleanValues.replacement_behavior);
  ...
  const [oBSERVATIONRecord, setOBSERVATIONRecord] = React.useState(oBSERVATION);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(OBSERVATION, idProp)
        : oBSERVATION;
      setOBSERVATIONRecord(record);
    };
    queryData();
  }, [idProp, oBSERVATION]);
  React.useEffect(resetStateValues, [oBSERVATIONRecord]);
  const validations = {
    observation_id: [],
    observation_dt: [],
    frequency: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            OBSERVATION.copyOf(oBSERVATIONRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "OBSERVATIONUpdateForm")}
      {...rest}
    >
      <TextField
